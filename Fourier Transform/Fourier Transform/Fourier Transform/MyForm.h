...
#pragma endregion
	...
	{
		//	filter paras
		double cutoff = 12.0, n = 2.0;
		//
		int w = dataManager->GetImageWidth();
		int h = dataManager->GetImageHeight();
		//
		double radius;
		double mid_h = (static_cast<double>(h) / 2.0), mid_w = (static_cast<double>(w) / 2.0);
		double dif_h, dif_w;
		...
		for (int i = 0; i < h; i++)
		{
			dif_h = static_cast<double>(i) - mid_h;
			for (int j = 0; j < w; j++)
			{
				dif_w = static_cast<double>(j) - mid_w;
				radius = sqrt((dif_h*dif_h) + (dif_w*dif_w));
				double rate = (1.0 + pow((radius / cutoff), 2.0*n));
				dataManager->FreqImag[i][j] = dataManager->FreqImag[i][j] / rate;
				dataManager->FreqReal[i][j] = dataManager->FreqReal[i][j] / rate;
				double ans = static_cast<double>(dataManager->InputImage[i][j]) / rate;
				...
				dataManager->OutputImage[i][j] = ans;
				IDFTImage->SetPixel(j, i, Color::FromArgb(ans, ans, ans));
			}
		}
		pictureBox_OutputImage->Image = IDFTImage;
	}
	private: System::Void highpassFilterToolStripMenuItem_Click(System::Object^  sender, System::EventArgs^  e)
	{
		...
		for (int i = 0; i < h; i++)
		{
			...
			for (int j = 0; j < w; j++)
			{
				...
				radius = sqrt((dif_h*dif_h) + (dif_w*dif_w));
				double rate = (1.0 + pow((radius / cutoff), 2.0*n));
				dataManager->FreqImag[i][j] = dataManager->FreqImag[i][j] - dataManager->FreqImag[i][j] / rate;
				dataManager->FreqReal[i][j] = dataManager->FreqReal[i][j] - dataManager->FreqReal[i][j] / rate;
				double ans = static_cast<double>(dataManager->InputImage[i][j]) - static_cast<double>(dataManager->InputImage[i][j]) / rate;
				///double ans = static_cast<double>(dataManager->InputImage[i][j]) - static_cast<double>(dataManager->InputImage[i][j]) / (1.0 + pow((radius / cutoff), 2.0*n));
